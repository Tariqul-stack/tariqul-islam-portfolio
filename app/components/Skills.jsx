...
import { 
  SiJavascript, SiHtml5, SiCss, SiReact, SiNextdotjs, SiReactrouter,
  SiTailwindcss, SiDaisyui, SiMongodb, SiGit, SiGithub, SiGoogle
} from 'react-icons/si';
import { TbChartBar, TbApi, TbComponents, TbBell } from 'react-icons/tb';
import { MdSecurity } from 'react-icons/md';

const skillsData = [
  { name: 'JavaScript (ES6+)', category: 'Languages', icon: SiJavascript, color: 'text-yellow-400' },
  { name: 'HTML5', category: 'Languages', icon: SiHtml5, color: 'text-orange-500' },
  { name: 'CSS3', category: 'Languages', icon: SiCss, color: 'text-blue-500' },
  { name: 'React.js', category: 'Frameworks', icon: SiReact, color: 'text-cyan-400' },
  { name: 'Next.js', category: 'Frameworks', icon: SiNextdotjs, color: 'text-[var(--text-primary)]' },
  { name: 'React Router', category: 'Frameworks', icon: SiReactrouter, color: 'text-red-500' },
  { name: 'Context API', category: 'Frameworks', icon: TbApi, color: 'text-indigo-400' },
  { name: 'Tailwind CSS', category: 'Styling', icon: SiTailwindcss, color: 'text-teal-400' },
  { name: 'DaisyUI', category: 'Styling', icon: SiDaisyui, color: 'text-teal-300' },
  { name: 'Better Auth', category: 'Database & Auth', icon: MdSecurity, color: 'text-teal-500' },
  { name: 'MongoDB', category: 'Database & Auth', icon: SiMongodb, color: 'text-green-500' },
  { name: 'Google OAuth', category: 'Database & Auth', icon: SiGoogle, color: 'text-blue-500' },
  { name: 'Recharts', category: 'Tools', icon: TbChartBar, color: 'text-teal-400' },
  { name: 'React Toastify', category: 'Tools', icon: TbBell, color: 'text-orange-400' },
  { name: 'Sonner', category: 'Tools', icon: TbBell, color: 'text-yellow-400' },
  { name: 'Lucide React', category: 'Tools', icon: TbComponents, color: 'text-[var(--text-primary)]' },
  { name: 'Git', category: 'Tools', icon: SiGit, color: 'text-orange-600' },
  { name: 'GitHub', category: 'Tools', icon: SiGithub, color: 'text-[var(--text-primary)]' },
];

const categories = ['All', 'Languages', 'Frameworks', 'Styling', 'Database & Auth', 'Tools'];
  ...
  return (
    <section id="skills" className="relative pt-28 md:pt-32 pb-24 overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {filteredSkills.map((skill, index) => (
                ...
                >
                  <skill.icon className={`text-5xl mb-4 ${skill.color} group-hover:scale-110 transition-transform duration-300`} />
                  <h4 className="text-[var(--text-primary)] font-medium mb-1">{skill.name}</h4>
        ...
        >
          <h3 className="text-xl font-bold text-[#2dd4bf] mb-6">Currently Exploring 🚀</h3>
          <div className="flex flex-wrap justify-center gap-4">
            {['Next.js Advanced Patterns', 'BetterAuth Deep Dive', 'Full-Stack Architecture'].map((item, index) => (
              <motion.div 
                key={item} 
                initial={{ opacity: 0, scale: 0.9 }}
            ...
            {skillBars.map((skill, index) => (
              <div key={skill.name} className="glass-card relative p-5">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-[#2dd4bf] font-medium">{skill.percentage}%</span>
                </div>
